...
$outputDir = Join-Path $repoRoot "OtherTracker\lasot\lasot936\ECO"
...
$cleanupTargets = @(
    ...
    (Join-Path $outputDir "tracking.stdout.log"),
    (Join-Path $outputDir "tracking.stderr.log"),
    (Join-Path $outputDir "tracking.pid.txt")
)
...
$oldProcs = Get-CimInstance Win32_Process | Where-Object {
    $_.CommandLine -like "*eco_default_lasot_headtail40*" -or $_.CommandLine -like "*eco_default_lasot*"
}
...
from pytracking.experiments.myexperiments import eco_default_lasot_headtail40

_, dataset = eco_default_lasot_headtail40()
base = Path("OtherTracker/pytracking/pytracking/tracking_results/eco/default")
...
cmd /c "set PYTHONUNBUFFERED=1 && set PYTHONWARNINGS=ignore::FutureWarning && $pythonExe -u $runExperiment myexperiments eco_default_lasot_headtail40 --debug 0 --threads 0 2>&1" |
...
trackers, dataset = eco_default_lasot_headtail40()
tracker = trackers[0]
report_name = "lasot_headtail40_eco_othertracker"
...
summary_path = repo / "OtherTracker" / "lasot" / "lasot936" / "ECO" / "summary.csv"
