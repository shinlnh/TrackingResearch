...
def _rfft2(a: torch.Tensor) -> torch.Tensor:
    return torch.view_as_real(torch.fft.rfftn(a, dim=(-2, -1)))


def _irfft2(a: torch.Tensor, signal_sizes=None) -> torch.Tensor:
    target_sizes = None if signal_sizes is None else tuple(int(v) for v in signal_sizes)
    return torch.fft.irfftn(torch.view_as_complex(a.contiguous()), s=target_sizes, dim=(-2, -1)).real
...
def cifft2(a, signal_sizes=None):
    ...
    return _irfft2(irfftshift2(a), signal_sizes=signal_sizes)
