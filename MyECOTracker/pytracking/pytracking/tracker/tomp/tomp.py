...
class ToMP(BaseTracker):
    ...
    def _autocast_context(self):
        if self.params.use_gpu and torch.cuda.is_available() and self.params.get('use_amp', False):
            amp_dtype = torch.float16 if self.params.get('amp_dtype', 'float16') == 'float16' else torch.bfloat16
            return torch.autocast(device_type='cuda', dtype=amp_dtype)
        return nullcontext()

    def _can_cache_train_side(self):
        return not self.params.get('update_classifier', False)

    def _clear_static_train_cache(self):
        self._cached_train_head_feat = None
        self._cached_train_ltrb = None

    def _ensure_static_train_cache(self):
        if not self._can_cache_train_side():
            return
        if self._cached_train_head_feat is not None and self._cached_train_ltrb is not None:
            return
        ...
        with torch.inference_mode():
            with self._autocast_context():
                self._cached_train_head_feat = self.net.head.extract_head_feat(train_samples)
            self._cached_train_ltrb = self.encode_bbox(target_boxes)
    ...
    def initialize_features(self):
        if not getattr(self, 'features_initialized', False):
            if self.params.use_gpu and torch.cuda.is_available():
                ...
                if self.params.has('matmul_precision'):
                    torch.set_float32_matmul_precision(self.params.matmul_precision)
            self.params.net.initialize()
            if self.params.get('use_torch_compile', False):
                compile_mode = self.params.get('torch_compile_mode', 'reduce-overhead')
                try:
                    self.params.net.net = torch.compile(self.params.net.net, mode=compile_mode)
                except Exception:
                    pass
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        self._clear_static_train_cache()
        ...
        self.last_object_presence_score = 1.0
    ...
    def encode_bbox(self, bbox):
        ...
        cache_device = getattr(self, '_encode_cache_device', None)
        cache_output_sz = getattr(self, '_encode_cache_output_sz', None)
        cache_stride = getattr(self, '_encode_cache_stride', None)
        if cache_device != bbox.device or cache_output_sz != output_sz or cache_stride != stride:
            ...
            self._encode_xs = locations[:, 0]
            self._encode_ys = locations[:, 1]
            self._encode_cache_device = bbox.device
            self._encode_cache_output_sz = output_sz
            self._encode_cache_stride = stride

        xs, ys = self._encode_xs, self._encode_ys
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        cheap_interval = self.params.get('cheap_frame_interval', 0)
        if cheap_interval and cheap_interval > 0 and self.frame_num % cheap_interval == 0:
            quick_state = torch.cat((self.pos[[1, 0]] - (self.target_sz[[1, 0]] - 1) / 2, self.target_sz[[1, 0]])).tolist()
            return {'target_bbox': quick_state,
                    'object_presence_score': self.last_object_presence_score}
        ...
        if self.params.get('fast_cls_only', False):
            new_pos = sample_pos[scale_ind, :] + translation_vec
            bbox_raw = torch.cat((new_pos[[1, 0]] - (self.target_sz[[1, 0]] - 1) / 2, self.target_sz[[1, 0]]))
        else:
        ...
        self.last_object_presence_score = out['object_presence_score']
    ...
    def direct_bbox_regression(self, bbox_preds, sample_coords, score_loc, scores_raw):
        ...
        cache_key = (device.type, device.index, int(self.img_sample_sz[0].item()), int(self.img_sample_sz[1].item()), stride, s1, s2)
        if getattr(self, '_bbox_grid_cache_key', None) != cache_key:
            shifts_x = torch.arange(
                0, int(self.img_sample_sz[0].item()), step=stride,
                dtype=torch.float32, device=device
            )
            shifts_y = torch.arange(
                0, int(self.img_sample_sz[1].item()), step=stride,
                dtype=torch.float32, device=device
            )
            shift_y, shift_x = torch.meshgrid(shifts_y, shifts_x, indexing='ij')
            shift_x = shift_x.reshape(-1)
            shift_y = shift_y.reshape(-1)
            locations = torch.stack((shift_x, shift_y), dim=1) + stride // 2
            xs = locations[:, 0].reshape(s1, s2)
            ys = locations[:, 1].reshape(s1, s2)
            self._bbox_grid_xs = xs
            self._bbox_grid_ys = ys
            self._bbox_grid_cache_key = cache_key

        xs, ys = self._bbox_grid_xs, self._bbox_grid_ys
        s1, s2 = scores_raw.shape[2:]
        train_img_sample_sz = self.train_img_sample_sz.to(device)
        ltrb = bbox_preds.permute(0,1,3,4,2)[0,0] * train_img_sample_sz[[0, 1, 0, 1]]
        xs1 = xs - ltrb[:, :, 0]
        xs2 = xs + ltrb[:, :, 2]
        ys1 = ys - ltrb[:, :, 1]
        ys2 = ys + ltrb[:, :, 3]
        sl = score_loc.long().to(device)
        ...
        return torch.stack([x1, y1, w, h]).float().cpu()
    ...
    def classify_target(self, sample_x: TensorList):
        ...
        with torch.inference_mode():
            ...
            with self._autocast_context():
                ...
                if self._can_cache_train_side():
                    self._ensure_static_train_cache()
                    train_feat = self._cached_train_head_feat
                    train_ltrb = self._cached_train_ltrb
                else:
                    train_feat = self.net.head.extract_head_feat(train_samples)
                    train_ltrb = self.encode_bbox(target_boxes)
                if self.params.get('fast_cls_only', False):
                    cls_weights, _, cls_test_feat_enc = self.net.head.get_filter_and_features(
                        train_feat, test_feat, train_label=target_labels, train_ltrb_target=train_ltrb
                    )
                    ...
                    cls_weights, bbreg_weights, cls_test_feat_enc, bbreg_test_feat_enc = \
                        self.net.head.get_filter_and_features_in_parallel(
                            train_feat, test_feat, num_gth_frames=self.num_gth_frames,
