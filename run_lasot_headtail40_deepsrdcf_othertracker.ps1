...
$outputDir = Join-Path $repoRoot "OtherTracker\lasot\lasot936\DeepSRDCF"
...
$oldProcs = Get-CimInstance Win32_Process | Where-Object {
    $_.CommandLine -like "*deepsrdcf_lasot_headtail40*"
}
...
from pytracking.experiments.myexperiments import deepsrdcf_lasot_headtail40

_, dataset = deepsrdcf_lasot_headtail40()
base = Path("OtherTracker/pytracking/pytracking/tracking_results/eco/deepsrdcf")
...
cmd /c "set PYTHONUNBUFFERED=1 && set PYTHONWARNINGS=ignore::FutureWarning && $pythonExe -u $runExperiment myexperiments deepsrdcf_lasot_headtail40 --debug 0 --threads 0 2>&1" |
...
trackers, dataset = deepsrdcf_lasot_headtail40()
tracker = trackers[0]
report_name = "lasot_headtail40_deepsrdcf_othertracker"
...
summary_path = repo / "OtherTracker" / "lasot" / "lasot936" / "DeepSRDCF" / "summary.csv"
