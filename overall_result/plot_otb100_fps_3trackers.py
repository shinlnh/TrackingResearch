"""Plot OTB100 challenge-representative FPS comparisons for three trackers.

Mirrors ``plot_otb100_challenge_representative_3trackers.py`` but compares
FPS instead of AUC, between the PC MyTracker, OSTrack-384, and STARK-ST101.
Left panels show per-frame instantaneous FPS, right panels show per-sequence
mean FPS bar charts.
"""
...
OUT_PNG = OUT_DIR / "otb100_fps_challenge_representative_mytracker_vs_ostrack_vs_stark.png"
OUT_CSV = OUT_DIR / "otb100_fps_challenge_representative_mytracker_vs_ostrack_vs_stark.csv"
...
def plot() -> None:
    ...
    fig.suptitle(
        "MyTracker vs OSTrack vs STARK FPS on Representative OTB100 Challenge Sequences (PC)",
        fontsize=14,
        fontweight="bold",
        y=0.985,
    )
    ...
    for row_idx, spec in enumerate(SEQUENCES):
        ...
        for tracker in TRACKERS:
            ...
            ax_line.plot(frames, fps[1:], color=tracker.color, linewidth=0.7, label=tracker.label)
            ax_line.axhline(
                mean_val,
                color=tracker.color,
                linestyle="--",
