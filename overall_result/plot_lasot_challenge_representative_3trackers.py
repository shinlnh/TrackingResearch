"""Plot LaSOT challenge-representative comparisons for three trackers.

The figure mirrors ``lasot_challenge_representative_mytracker_vs_csrt.png``:
left panels show per-frame IoU against ground truth, right panels show
bounding-box center trajectories on the first frame. MyTracker uses the
local/PC full-tracker result bundle.
"""
...
OUT_PNG = OUT_DIR / "lasot_challenge_representative_myeco_vs_ostrack_vs_stark.png"
OUT_CSV = OUT_DIR / "lasot_challenge_representative_myeco_vs_ostrack_vs_stark.csv"
...
def lasot_paths(sequence: str) -> tuple[Path, Path]:
...
def plot() -> None:
    ...
    for row_idx, spec in enumerate(SEQUENCES):
        ...
        gt_path, image_path = lasot_paths(sequence)
