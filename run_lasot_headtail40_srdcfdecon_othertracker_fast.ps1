...
$outputDir = Join-Path $repoRoot "OtherTracker\lasot\lasot936\SRDCFdecon"
$resultsDir = Join-Path $outputDir "tracking_results\SRDCFdecon"
$logPath = Join-Path $outputDir "tracking.log"
$summaryPath = Join-Path $outputDir "summary.csv"
$workerLogDir = Join-Path $outputDir "worker_logs"
...
$cleanupTargets = @(
    ...
    (Join-Path $outputDir "srdcfdecon_matlab_summary.csv"),
    $workerLogDir
)
...
New-Item -ItemType Directory -Force $workerLogDir | Out-Null

@'
import pathlib

seq_file = pathlib.Path("OtherTracker/lasot/lasot936/headtail40_sequences.txt")
results_dir = pathlib.Path("OtherTracker/lasot/lasot936/SRDCFdecon/tracking_results/SRDCFdecon")
...
'@ | & $pythonExe - | Tee-Object -FilePath $logPath

$sequenceCount = (Get-Content $sequenceFile | Where-Object { $_.Trim() -ne "" }).Count
if ($sequenceCount -le 0) {
    throw "Sequence list is empty: $sequenceFile"
}

$Workers = [Math]::Max(1, [Math]::Min($Workers, $sequenceCount))
$chunkSize = [Math]::Ceiling($sequenceCount / $Workers)

$jobs = @()
for ($workerIndex = 0; $workerIndex -lt $Workers; $workerIndex++) {
    $startIndex = 1 + ($workerIndex * $chunkSize)
    $endIndex = [Math]::Min($sequenceCount, $startIndex + $chunkSize - 1)
    if ($startIndex -gt $sequenceCount) {
        break
    }

    $workerId = $workerIndex + 1
    $workerLog = Join-Path $workerLogDir ("worker_{0:D2}.log" -f $workerId)
    $matlabCommand = "addpath(fullfile(pwd,'OtherTracker','SRDCFdecon')); run_srdcfdecon_lasot_headtail40([], [], [], $startIndex, $endIndex);"
    $cmdLine = "set PYTHONUNBUFFERED=1 && set SRDCFDECON_SKIP_MATLAB_SUMMARY=1 && matlab -singleCompThread -batch ""$matlabCommand"" > ""$workerLog"" 2>&1"

    $process = Start-Process -FilePath "cmd.exe" -ArgumentList "/c", $cmdLine -WorkingDirectory $repoRoot -PassThru
    $jobs += [pscustomobject]@{
        Worker = $workerId
    ...
    $startLine = "[start] worker {0}/{1} sequences {2}-{3} pid={4}" -f $workerId, $Workers, $startIndex, $endIndex, $process.Id
    $startLine | Tee-Object -FilePath $logPath -Append | Out-Null
}

while (($jobs | Where-Object { -not $_.Process.HasExited }).Count -gt 0) {
    foreach ($job in $jobs) {
        if (-not $job.Process.HasExited) {
            $statusLine = "[wait ] worker {0} sequences {1}-{2} pid={3}" -f $job.Worker, $job.StartIndex, $job.EndIndex, $job.Process.Id
...
$runExitCode = 0
foreach ($job in $jobs) {
    if ($job.Process.ExitCode -ne 0 -and $runExitCode -eq 0) {
        $runExitCode = $job.Process.ExitCode
    }

    $finishLine = "[done ] worker {0} sequences {1}-{2} exit={3}" -f $job.Worker, $job.StartIndex, $job.EndIndex, $job.Process.ExitCode
    $finishLine | Tee-Object -FilePath $logPath -Append | Out-Null
    Add-Content -LiteralPath $logPath -Value ""
    Add-Content -LiteralPath $logPath -Value ("===== WORKER {0:D2} [{1}-{2}] =====" -f $job.Worker, $job.StartIndex, $job.EndIndex)
    if (Test-Path $job.LogPath) {
        Get-Content $job.LogPath | Add-Content -LiteralPath $logPath
    }
}
...
tracker = SimpleNamespace(
    ...
    run_id=None,
    display_name="SRDCFdecon",
    results_dir=str(repo / "OtherTracker" / "lasot" / "lasot936" / "SRDCFdecon" / "tracking_results" / "SRDCFdecon"),
)

report_name = "lasot_headtail40_srdcfdecon_othertracker"
...
summary_path = repo / "OtherTracker" / "lasot" / "lasot936" / "SRDCFdecon" / "summary.csv"
