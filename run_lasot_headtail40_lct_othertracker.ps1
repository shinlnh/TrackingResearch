...
$outputDir = Join-Path $repoRoot "OtherTracker\lasot\lasot936\LCT"
$resultsDir = Join-Path $outputDir "tracking_results\LCT"
...
$cleanupTargets = @(
    ...
    (Join-Path $outputDir "lct_matlab_summary.csv")
)
...
results_dir = pathlib.Path("OtherTracker/lasot/lasot936/LCT/tracking_results/LCT")
...
cmd /c "set PYTHONUNBUFFERED=1 && matlab -batch ""addpath(fullfile(pwd,'OtherTracker','LCT')); run_lct_lasot_headtail40;"" 2>&1" |
...
tracker = SimpleNamespace(
    ...
    run_id=None,
    display_name="LCT",
    results_dir=str(repo / "OtherTracker" / "lasot" / "lasot936" / "LCT" / "tracking_results" / "LCT"),
)

report_name = "lasot_headtail40_lct_othertracker"
...
summary_path = repo / "OtherTracker" / "lasot" / "lasot936" / "LCT" / "summary.csv"
