...
$runner = Join-Path $repoRoot "OtherTracker\KCF\run_kcf_lasot.py"
$sequenceFile = Join-Path $repoRoot "OtherTracker\lasot\lasot936\headtail40_sequences.txt"
$outputDir = Join-Path $repoRoot "OtherTracker\lasot\lasot936\KCF"
$resultsDir = Join-Path $outputDir "tracking_results\KCF"
...
results_dir = pathlib.Path("OtherTracker/lasot/lasot936/KCF/tracking_results/KCF")
...
cmd /c "set PYTHONUNBUFFERED=1 && $pythonExe -u $runner --sequence-file $sequenceFile --output-dir $outputDir --display-name KCF --tracker-name kcf 2>&1" |
