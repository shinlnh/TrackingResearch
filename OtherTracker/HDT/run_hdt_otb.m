function summary = run_hdt_otb(otb_root, sequence_file, out_dir, start_index, end_index)
%RUN_HDT_OTB Benchmark HDT on OTB using the bundled MATLAB implementation.
...
if nargin < 3 || isempty(out_dir)
    out_dir = fullfile(repo_root, 'OtherTracker', 'HDT', 'otb100_fps');
end
...
opts = struct();
opts.tracker_name = 'HDT';
opts.tracker_path = fullfile(fileparts(mfilename('fullpath')), 'HDT');
opts.setup_func = 'setup_paths';
opts.main_func = 'run_HDT_benchmark';
