function [positions] = tracker_ensemble(img_files, pos, target_sz, ...
                                                ...
                                                cell_size, show_visualization, rect_anno, bSaveImage, pathModel, varargin)

progress_name = 'sequence';
if ~isempty(varargin) && ~isempty(varargin{1})
    progress_name = varargin{1};
end
...
for frame = 1:num_frames,
    ...
    if should_log_progress(frame, num_frames, last_log_time, log_frame_interval, log_time_interval)
        elapsed_sec = toc(sequence_timer);
        avg_fps = frame / max(elapsed_sec, eps);
        remaining_frames = max(num_frames - frame, 0);
        eta_sec = remaining_frames / max(avg_fps, eps);
        fprintf('[HDT] %s frame %d/%d (%.1f%%%%) elapsed %.1fs avg_fps %.3f eta %.1fs\n', ...
            progress_name, frame, num_frames, 100 * frame / num_frames, elapsed_sec, avg_fps, eta_sec);
        last_log_time = tic;
    end
...
function tf = should_log_progress(frame, num_frames, last_log_time, log_frame_interval, log_time_interval)
tf = frame == 1 || ...
     frame == num_frames || ...
     mod(frame, log_frame_interval) == 0 || ...
     toc(last_log_time) >= log_time_interval;
end
