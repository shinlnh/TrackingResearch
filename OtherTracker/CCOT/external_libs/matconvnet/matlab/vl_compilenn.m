...
if opts.enableGpu
  ...
  switch arch
    ...
    case 'win64'
      flags.link{end+1} = '-lgpumexbinder' ;
  end
  if opts.enableCudnn
...
flags.mexcu= horzcat({'-f' mex_cuda_config(root)}, ...
                     flags.cc, ...
                     {'-R2018a'}, ...
                     {['CXXFLAGS=$CXXFLAGS -I"' matlab_gpu_include_dir() '" ' quote_nvcc(flags.ccpass) ' ' strjoin(flags.nvccpass)]}, ...
...
arch = computer('arch') ;
  switch arch
    case {'win64'}
      config_dir = matlab_gpu_config_dir(arch) ;
    case {'maci64', 'glnxa64'}
...
error('Could not locate the MATLAB GPU include directory.') ;

% --------------------------------------------------------------------
function config_dir = matlab_gpu_config_dir(arch)
% --------------------------------------------------------------------
candidates = { ...
...
error('Could not locate the MATLAB CUDA mex configuration directory.') ;
...
if ~isempty(cl_candidates)
  cl_path = cl_candidates(1).folder;
else
