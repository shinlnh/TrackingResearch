...
for k = 1:length(fparams.output_layer)
    if fparams.downsample_factor(k) == 1
        feat = cnn_feat(fparams.output_layer(k) + 1).x(fparams.start_ind(k,1):fparams.end_ind(k,1), fparams.start_ind(k,2):fparams.end_ind(k,2), :, :);
    else
        feat = average_feature_region(cnn_feat(fparams.output_layer(k) + 1).x(fparams.start_ind(k,1):fparams.end_ind(k,1), fparams.start_ind(k,2):fparams.end_ind(k,2), :, :), fparams.downsample_factor(k));
    end

    if isa(feat, 'gpuArray')
        feat = gather(feat);
    end
    feature_map{k} = feat;
end
