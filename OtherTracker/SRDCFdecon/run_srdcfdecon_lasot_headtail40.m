function summary = run_srdcfdecon_lasot_headtail40(lasot_root, sequence_file, out_dir, start_index, end_index)
%RUN_SRDCFDECON_LASOT_HEADTAIL40 Run SRDCFdecon on the LaSOT head/tail 40 subset.
...
if nargin < 3 || isempty(out_dir)
    out_dir = fullfile(repo_root, 'OtherTracker', 'lasot', 'lasot936', 'SRDCFdecon');
end
...
tracker_root = fullfile(fileparts(mfilename('fullpath')), 'SRDCFdecon');
result_dir = fullfile(out_dir, 'tracking_results', 'SRDCFdecon');
...
if isempty(parts)
    error('run_srdcfdecon_lasot_headtail40:invalidSequence', 'Invalid sequence name: %s', seq_name);
end
...
if ~exist(gt_path, 'file')
    error('run_srdcfdecon_lasot_headtail40:missingGt', 'Missing groundtruth for %s', seq_name);
end
...
if fid == -1
    error('run_srdcfdecon_lasot_headtail40:sequenceFile', 'Could not open sequence file: %s', sequence_file);
end
...
if fid == -1
    error('run_srdcfdecon_lasot_headtail40:summaryWrite', 'Could not write summary CSV: %s', csv_path);
end
cleanup_obj = onCleanup(@() fclose(fid)); %#ok<NASGU>
