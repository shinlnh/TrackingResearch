...
% toc
    ...
    if ~config.display
        done = frame_id - start_frame + 1;
        should_log = done == 1 || frame_id == end_frame || mod(done, 25) == 0;
        if should_log
            elapsed_wall = toc(wall_timer);
            avg_fps = done / max(elapsed_wall, eps);
            eta_sec = (end_frame - frame_id) / max(avg_fps, eps);
            fprintf('[MEEM] frame %d/%d (%.1f%%) elapsed %.1fs avg_fps %.3f eta %.1fs\n', ...
                frame_id, end_frame, 100 * done / max(result.len, 1), ...
                elapsed_wall, avg_fps, eta_sec);
        end
    end
end
