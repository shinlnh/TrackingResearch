function summary = run_dsst_lasot_headtail40(lasot_root, sequence_file, out_dir, start_index, end_index)
%RUN_DSST_LASOT_HEADTAIL40 Run DSST on the LaSOT head/tail 40 subset.
...
if nargin < 3 || isempty(out_dir)
    out_dir = fullfile(repo_root, 'OtherTracker', 'lasot', 'lasot936', 'DSST');
end
...
result_dir = fullfile(out_dir, 'tracking_results', 'DSST');
...
write_summary_csv(fullfile(out_dir, 'dsst_matlab_summary.csv'), summary);
end

function seq = build_lasot_sequence(lasot_root, seq_name)
parts = regexp(seq_name, '^([^-]+)-', 'tokens', 'once');
if isempty(parts)
    error('run_dsst_lasot_headtail40:invalidSequence', 'Invalid sequence name: %s', seq_name);
end
...
if ~exist(gt_path, 'file')
    error('run_dsst_lasot_headtail40:missingGt', 'Missing groundtruth for %s', seq_name);
end
...
if fid == -1
    error('run_dsst_lasot_headtail40:sequenceFile', 'Could not open sequence file: %s', sequence_file);
end
...
if fid == -1
    error('run_dsst_lasot_headtail40:summaryWrite', 'Could not write summary CSV: %s', csv_path);
end
cleanup_obj = onCleanup(@() fclose(fid)); %#ok<NASGU>
