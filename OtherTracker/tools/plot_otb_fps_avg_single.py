...
def plot_entries(entries: list[FpsEntry], out_png: Path, my_tracker_label: str = "MyTracker") -> None:
    ...
    colors = ["#f2c94c" if entry.tracker == my_tracker_label else "#ef7aa1" for entry in entries]
    ...
    ax.invert_yaxis()
    ax.set_xlabel("FPS")
    ax.set_title("OTB100 FPS Comparison")
...
def main() -> int:
    ...
    out_png = repo_root / "overall_result" / "picture" / "otb" / "fps_avg" / "fps_avg_mytracker_vs_othertracker.png"
