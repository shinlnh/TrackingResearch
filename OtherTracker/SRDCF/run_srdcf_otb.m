function summary = run_srdcf_otb(otb_root, sequence_file, out_dir, start_index, end_index)
%RUN_SRDCF_OTB Benchmark SRDCF on OTB using the original ICCV 2015 settings.
...
if nargin < 3 || isempty(out_dir)
    out_dir = fullfile(repo_root, 'OtherTracker', 'SRDCF', 'otb100_fps');
end
...
opts = struct();
opts.tracker_name = 'SRDCF';
opts.tracker_path = fullfile(fileparts(mfilename('fullpath')), 'SRDCF');
opts.setup_func = 'setup_paths';
opts.main_func = 'run_SRDCF';
