...
% Local overrides for runner-specific experiments.
if nargin >= 4 && ~isempty(parameters) && isstruct(parameters)
    override_fields = fieldnames(parameters);
    for field_idx = 1:numel(override_fields)
        params.(override_fields{field_idx}) = parameters.(override_fields{field_idx});
    end
end
