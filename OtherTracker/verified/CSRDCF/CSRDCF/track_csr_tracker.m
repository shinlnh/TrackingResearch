function [tracker, region] = track_csr_tracker(tracker, img)
    ...
    if c(1) + tracker.base_target_sz(1)/2 > size(img, 2)
        c(1) = size(img, 2) - tracker.base_target_sz(1)/2;
    end
    
    if c(2) + tracker.base_target_sz(2)/2 > size(img, 1)
        c(2) = size(img, 1) - tracker.base_target_sz(2)/2;
    end
