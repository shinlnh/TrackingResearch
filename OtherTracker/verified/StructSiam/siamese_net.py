...
class SiameseNet:
    ...
    def buildOriBranch(self, inputs, opts, isTrainingOp, branchName):
	    ...
	    outputs_p = outputs_p * float(int(outputs.get_shape()[3]))
            ...
            outputs_ = self.conv(outputs,384, 3, 1, 1, [1.0, 2.0], [1.0, 0.0], opts['trainWeightDecay'], opts['stddev'],name='crf_conv_u1')
            outputs_p = outputs_p + outputs_

            outputs = self.conv(outputs, 384, 3, 1, 1, [1.0, 2.0], [1.0, 0.0], opts['trainWeightDecay'], opts['stddev'])
            #outputs = outputs #+ outputs_p
            outputs = self.batchNorm(outputs, isTrainingOp)
            outputs = tf.nn.relu(outputs)
        ...
        with tf.variable_scope('scala4'):
            ...
            outputs_p = self.conv(outputs_p, 384, 3, 1, 2, [1.0, 2.0], [1.0, 0.0], opts['trainWeightDecay'],
                                  opts['stddev'], name='crf_conv_p2')

            outputs_p_ = self.conv(outputs_p_, 384, 3, 1, 2, [1.0, 2.0], [1.0, 0.0], opts['trainWeightDecay'],
                                  opts['stddev'], name='crf_conv_u1')
            outputs_p = outputs_p + outputs_p_
        ...
        with tf.variable_scope('scala5'):
            print("Building conv5...")
            # outputs = conv2(outputs, 192, 256, 3, 1)
            outputs_p = self.conv(outputs_p_, 256, 3, 1, 2, [1.0, 2.0], [1.0, 0.0], opts['trainWeightDecay'],
                                  opts['stddev'], name='crf_conv_p1', padding='SAME')
            ...
            outputs_p = self.conv(outputs_p, 256, 3, 1, 2, [1.0, 2.0], [1.0, 0.0], opts['trainWeightDecay'],
                                  opts['stddev'], name='crf_conv_p2')

            outputs_p_ = self.conv(outputs_p_, 256, 3, 1, 2, [1.0, 2.0], [1.0, 0.0], opts['trainWeightDecay'],
                                   opts['stddev'], name='crf_conv_u1')
            outputs_p = outputs_p + outputs_p_
            ...
            outputs = outputs + outputs_p_

        return outputs

    def conv(self, inputs, filters, size, stride, groups, lrs, wds, wd, stddev, name=None,padding=None):
