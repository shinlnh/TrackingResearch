...
for frame = 1:num_frames,
    ...
    if visualization == 0
        if frame == 1
            if isempty(dsst_sequence_name)
                seq_name = 'DSST';
            else
                seq_name = dsst_sequence_name;
            end
            fprintf('[DSST] %s start frames=%d\n', seq_name, num_frames);
        end

        should_log = frame == 1 || frame == num_frames || mod(frame, 25) == 0;
        if should_log
            elapsed_wall = toc(wall_timer);
            avg_fps_wall = frame / max(elapsed_wall, eps);
            eta_sec = (num_frames - frame) / max(avg_fps_wall, eps);
            fprintf('[DSST] %s frame %d/%d (%.1f%%) elapsed %.1fs avg_fps %.3f eta %.1fs\n', ...
                dsst_sequence_name, frame, num_frames, 100 * frame / max(num_frames, 1), ...
                elapsed_wall, avg_fps_wall, eta_sec);
        end
    end
