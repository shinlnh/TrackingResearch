...
{
	// read config file
	string configPath = "./Struck/config.txt";
	if (argc > 1)
	{
		configPath = argv[1];
	}
	...
	// parse frames file
	startFrame = conf.startFrame;
	endFrame = conf.endFrame;
	/*
	...
	*/
	...
	imgFormat = conf.sequenceBasePath+"/img/%08d.jpg";
	...
	string gtFilePath = conf.sequenceBasePath+"/groundtruth.txt";
