...
MV_LEN = 6;
mean_afnv(~isfinite(mean_afnv)) = 0;
std_afnv(~isfinite(std_afnv)) = 0;
std_afnv = max(std_afnv, 0);

% The scale terms are represented in log-space below and must stay positive.
mean_afnv(:, 1) = max(mean_afnv(:, 1), eps);
mean_afnv(:, 4) = max(mean_afnv(:, 4), eps);
