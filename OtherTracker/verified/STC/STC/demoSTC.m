...
% Date: 11/24/2013
%%
function rect_result = demoSTC(tmp_video_path, tmp_video)
...
%     fprintf('processing %d/%d \n', frame, numel(img_dir));
    ...
    window = window/sum(sum(window));%normalization
 	%load image
    img = imread([tmp_video_path '/img/' img_dir(frame).name]);	    
	if size(img,3) > 1,
		im = rgb2gray(img);
	end
    ...
    if target_sz(1) > 3.5 * ori_target_sz(1)
        target_sz(1) = 3.5 * ori_target_sz(1);
    end
    if target_sz(2) > 3.5 * ori_target_sz(2)
        target_sz(2) = 3.5 * ori_target_sz(2);
    end
...
%     drawnow;    
    ...
    if frame > 1
        rect_result(frame, :) = rect_position;
    end
end
end