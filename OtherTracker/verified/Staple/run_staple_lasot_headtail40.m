function summary = run_staple_lasot_headtail40(lasot_root, sequence_file, out_dir, start_index, end_index)
%RUN_STAPLE_LASOT_HEADTAIL40 Run Staple on the LaSOT head/tail 40 subset.
...
if nargin < 3 || isempty(out_dir)
    out_dir = fullfile(repo_root, 'OtherTracker', 'lasot', 'lasot936', 'Staple');
end
...
result_dir = fullfile(out_dir, 'tracking_results', 'Staple');
...
for idx = start_index:end_index
    ...
    try
        cd(tracker_root);
        setup_paths();
        res = run_Staple_benchmark(seq, '', false);
        path(old_path);
        cd(old_dir);
    catch err
        path(old_path);
...
write_summary_csv(fullfile(out_dir, 'staple_matlab_summary.csv'), summary);
end

function seq = build_lasot_sequence(lasot_root, seq_name)
parts = regexp(seq_name, '^([^-]+)-', 'tokens', 'once');
if isempty(parts)
    error('run_staple_lasot_headtail40:invalidSequence', 'Invalid sequence name: %s', seq_name);
end
...
if ~exist(gt_path, 'file')
    error('run_staple_lasot_headtail40:missingGt', 'Missing groundtruth for %s', seq_name);
end
...
if fid == -1
    error('run_staple_lasot_headtail40:sequenceFile', 'Could not open sequence file: %s', sequence_file);
end
...
if fid == -1
    error('run_staple_lasot_headtail40:summaryWrite', 'Could not write summary CSV: %s', csv_path);
end
cleanup_obj = onCleanup(@() fclose(fid)); %#ok<NASGU>
