...
%   sets up the parameters and loads the video information.
    ...
    global lct_sequence_name;
    seq_name = lct_sequence_name;
    if isempty(seq_name)
        seq_name = 'sequence';
    end
    fprintf('[LCT] %s start frames=%d\n', seq_name, numel(img_files));
    sequence_timer = tic;
...
%     max_scale_factor = scale_step ^ floor(log(min([size(im,1) size(im,2)] ./ base_target_sz)) / log(scale_step));
	...
	for frame = 1:numel(img_files),
        ...
        if should_log
            ...
            fprintf('[LCT] %s frame %d/%d (%.1f%%) elapsed %.1fs avg_fps %.3f eta %.1fs\n', ...
