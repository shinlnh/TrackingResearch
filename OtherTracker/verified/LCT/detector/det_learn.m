...
%     labels=[labels; svm_struct.GroupNames(svm_struct.SupportVectorIndices)];  
    ...
    if exist('vl_svmtrain', 'file') == 2 || exist('vl_svmtrain', 'file') == 3
        [svm_struct.w, svm_struct.b] = vl_svmtrain(feat, labels, 0.5);
    else
        persistent warned_missing_vl_svmtrain
        if isempty(warned_missing_vl_svmtrain)
            warned_missing_vl_svmtrain = true;
            warning('LCT:MissingVlSvmtrain', ...
                'vl_svmtrain not found, using onlineSVMTrain fallback for detector init.');
        end
        svm_struct.w = onlineSVMTrain(feat', labels, 0.5, []);
        svm_struct.b = 0;
    end
