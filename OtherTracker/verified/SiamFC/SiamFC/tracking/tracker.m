...
%         warning('No stats found at %s', p.stats_path);
    ...
    [imgFiles, targetPosition, targetSize] = load_video_info(p.video_path, p.video);
    ...
    im = gpuArray(single(imread(imgFiles{startFrame})));
    ...
    for i = startFrame:nImgs
        ...
        if i>startFrame
            ...
            im = gpuArray(single(imread(imgFiles{i})));
   			% if grayscale repeat one channel to match filters size
    		if(size(im, 3)==1)
        		im = repmat(im, [1 1 3]);
    		end
            ...
            targetPosition(1) = min(targetPosition(1), size(im, 1)-targetSize(1)/2);
            targetPosition(2) = min(targetPosition(2), size(im, 2)-targetSize(2)/2);
