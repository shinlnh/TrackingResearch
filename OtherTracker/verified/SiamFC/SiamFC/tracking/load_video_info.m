% -------------------------------------------------------------------------------------------------
function [imgs, pos, target_sz] = load_video_info(video_path, video)
%LOAD_VOT_VIDEO_INFO
...
%     imgs = vl_imreadjpeg(img_files,'numThreads', 12);
    ...
    cx = ground_truth(1,1)+round(ground_truth(1,3)/2);
    cy = ground_truth(1,2)+round(ground_truth(1,4)/2);
    w = ground_truth(1,3);
    h = ground_truth(1,4);
    ...
    tmp_imgs = dir([video_path '/img/*.jpg']);
    
    for i = 1:numel(tmp_imgs)
        imgs{i} = [video_path '/img/' tmp_imgs(i).name];
    end
end
