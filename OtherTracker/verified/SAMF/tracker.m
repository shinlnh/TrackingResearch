...
temp = load(fullfile(root_path, 'w2crs.mat'));
w2c = temp.w2crs;
...
% 	window_sz = 2 .^ nextpow2(window_sz);
    ...
    global samf_sequence_name;
    seq_name = samf_sequence_name;
    if isempty(seq_name)
        seq_name = 'sequence';
    end
    fprintf('[SAMF] %s start frames=%d\n', seq_name, numel(img_files));
    sequence_timer = tic;
    last_log_elapsed = -inf;
...
% 		patch = get_subwindow(im, pos, window_sz);
		...
		positions(frame,:) = pos;
        frame_elapsed = toc();
		time = time + frame_elapsed;

        elapsed = toc(sequence_timer);
        should_log = frame == 1 || frame == numel(img_files) || ...
            mod(frame, log_frame_interval) == 0 || ...
            (elapsed - last_log_elapsed) >= log_time_interval;
        if should_log
            avg_fps = frame / max(elapsed, eps);
            eta = (numel(img_files) - frame) / max(avg_fps, eps);
            fprintf('[SAMF] %s frame %d/%d (%.1f%%) elapsed %.1fs avg_fps %.3f eta %.1fs\n', ...
                seq_name, frame, numel(img_files), 100 * frame / numel(img_files), elapsed, avg_fps, eta);
...
% 			pause(0.05)  %uncomment to run slower
    ...
    fprintf('[SAMF] %s done total_time=%.1fs fps=%.4f\n', seq_name, time, numel(img_files) / max(time, eps));
end
