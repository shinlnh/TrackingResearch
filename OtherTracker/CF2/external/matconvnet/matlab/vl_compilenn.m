...
if opts.enableImreadJpeg
  ...
  switch arch
    ...
    case 'win64'
      gpu_lib = fullfile(matlabroot, 'bin', arch, 'gpu.lib');
      if exist(gpu_lib, 'file')
        flags.link{end+1} = ['-L"' fullfile(matlabroot, 'bin', arch) '"'];
        flags.link{end+1} = '-lgpu' ;
      else
        gpu_api_lib = fullfile(matlabroot, 'extern', 'lib', arch, 'microsoft', 'gpumexbinder.lib');
        if exist(gpu_api_lib, 'file')
          flags.linklibs{end+1} = ['"' gpu_api_lib '"'] ;
        else
          error('Unable to locate gpumexbinder.lib for MATLAB GPU linking.');
        end

        gpu_runtime_lib = fullfile(matlabroot, 'bin', arch, 'gpucoder_cuda.lib');
        if exist(gpu_runtime_lib, 'file')
          flags.linklibs{end+1} = ['"' gpu_runtime_lib '"'] ;
        end
      end
  end
  if opts.enableCudnn
...
switch arch
  ...
  case {'win64'}
    ...
    cl_path = check_clpath(); % check whether cl.exe in path
...
flags.nvcc = horzcat(flags.cc, ...
                     ...
                     {sprintf('-I"%s"', matlab_gpu_include_dir())}, ...
...
switch arch
  case {'win64'}
    config_dir = matlab_gpu_mex_config_dir(arch) ;
  case {'maci64', 'glnxa64'}
...
% --------------------------------------------------------------------
function include_dir = matlab_gpu_include_dir()
% --------------------------------------------------------------------
candidates = { ...
...
for i = 1:numel(candidates)
  if exist(fullfile(candidates{i}, 'gpu', 'mxGPUArray.h'), 'file')
    include_dir = candidates{i};
    return;
  end
end

error('Unable to locate MATLAB GPU include directory.');

% --------------------------------------------------------------------
function config_dir = matlab_gpu_mex_config_dir(arch)
% --------------------------------------------------------------------
candidates = { ...
  fullfile(matlabroot, 'toolbox', 'parallel', 'gpu', 'extern', 'src', 'mex', arch), ...
  fullfile(matlabroot, 'toolbox', 'distcomp', 'gpu', 'extern', 'src', 'mex', arch)};

config_dir = '';
for i = 1:numel(candidates)
  if exist(candidates{i}, 'dir')
    config_dir = candidates{i};
    return;
  end
end

error('Unable to locate MATLAB GPU mex configuration directory.');
...
cl_path = find_cl_path(cc);
[status, ~] = system('where cl.exe >nul 2>&1');
if status ~= 0
...
% --------------------------------------------------------------------
function cl_path = find_cl_path(cc)
% --------------------------------------------------------------------
cl_path = '';

if isfield(cc, 'Details') && isfield(cc.Details, 'CompilerExecutable')
  compiler_exe = cc.Details.CompilerExecutable;
  if exist(compiler_exe, 'file')
    cl_path = fileparts(compiler_exe);
    return;
  end
end

modern_glob = fullfile(cc.Location, 'VC', 'Tools', 'MSVC', '*', 'bin', 'Hostx64', 'x64', 'cl.exe');
modern = dir(modern_glob);
if ~isempty(modern)
  [~, idx] = max([modern.datenum]);
  cl_path = modern(idx).folder;
  return;
end

legacy = fullfile(cc.Location, 'VC', 'bin', 'amd64', 'cl.exe');
if exist(legacy, 'file')
  cl_path = fileparts(legacy);
  return;
end

error('Unable to locate cl.exe from the active mex configuration.');
...
for v = {'5.5', '6.0', '6.5', '7.0', '7.5', '8.0', '8.5', '9.0', ...
         '10.0', '10.1', '10.2', '11.0', '11.1', '11.2', '11.3', ...
         '11.4', '11.5', '11.6', '11.7', '11.8', '12.0', '12.1', ...
         '12.2', '12.3', '12.4', '13.0', '13.1'}
  switch computer('arch')
    case 'glnxa64'
...
if strcmp(computer('arch'), 'win64')
  paths{end+1} = fullfile(matlabroot, 'sys', 'cuda', 'win64', 'cuda', 'bin', 'nvcc.exe') ;
end
