function summary = run_cf2_lasot_headtail40(lasot_root, sequence_file, out_dir, start_index, end_index)
%RUN_CF2_LASOT_HEADTAIL40 Run CF2 on the LaSOT head/tail 40 subset.
...
if nargin < 3 || isempty(out_dir)
    out_dir = fullfile(repo_root, 'OtherTracker', 'lasot', 'lasot936', 'CF2');
end
...
result_dir = fullfile(out_dir, 'tracking_results', 'CF2');
...
maybe_warmup_gpu(tracker_root, lasot_root, sequence_names, start_index);
...
write_summary_csv(fullfile(out_dir, 'cf2_matlab_summary.csv'), summary);
end

function seq = build_lasot_sequence(lasot_root, seq_name)
parts = regexp(seq_name, '^([^-]+)-', 'tokens', 'once');
if isempty(parts)
    error('run_cf2_lasot_headtail40:invalidSequence', 'Invalid sequence name: %s', seq_name);
end
...
if ~exist(gt_path, 'file')
    error('run_cf2_lasot_headtail40:missingGt', 'Missing groundtruth for %s', seq_name);
end
...
if fid == -1
    error('run_cf2_lasot_headtail40:sequenceFile', 'Could not open sequence file: %s', sequence_file);
end
...
if fid == -1
    error('run_cf2_lasot_headtail40:summaryWrite', 'Could not write summary CSV: %s', csv_path);
end
cleanup_obj = onCleanup(@() fclose(fid)); %#ok<NASGU>
...
function maybe_warmup_gpu(tracker_root, lasot_root, sequence_names, start_index)
if ~strcmp(getenv('CF2_USE_GPU'), '1')
    return;
end

if isempty(sequence_names) || start_index > numel(sequence_names)
    return;
end

seq_name = sequence_names{start_index};
seq = build_lasot_sequence(lasot_root, seq_name);
warmup_frames = min(seq.len, 50);

if warmup_frames < 2
    return;
end

seq.s_frames = seq.s_frames(1:warmup_frames);
seq.len = warmup_frames;

fprintf('[warmup] CF2 GPU compile start %s frames=%d\n', seq_name, warmup_frames);
...
fprintf('[warmup] CF2 GPU ready %s fps=%.6f\n', seq_name, double(res.fps));
end
