...
$outputDir = Join-Path $repoRoot "OtherTracker\lasot\lasot936\SAMF"
$resultsDir = Join-Path $outputDir "tracking_results\SAMF"
...
$cleanupTargets = @(
    ...
    (Join-Path $outputDir "samf_matlab_summary.csv")
)
...
results_dir = pathlib.Path("OtherTracker/lasot/lasot936/SAMF/tracking_results/SAMF")
...
cmd /c "set PYTHONUNBUFFERED=1 && matlab -batch ""addpath(fullfile(pwd,'OtherTracker','SAMF')); run_samf_lasot_headtail40;"" 2>&1" |
...
tracker = SimpleNamespace(
    ...
    run_id=None,
    display_name="SAMF",
    results_dir=str(repo / "OtherTracker" / "lasot" / "lasot936" / "SAMF" / "tracking_results" / "SAMF"),
)

report_name = "lasot_headtail40_samf_othertracker"
...
summary_path = repo / "OtherTracker" / "lasot" / "lasot936" / "SAMF" / "summary.csv"
