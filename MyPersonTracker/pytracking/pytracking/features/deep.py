...
class ResNet18m1(MultiFeatureBase):
    ...
    def initialize(self):
        ...
        if self.use_gpu:
            ...
            if hasattr(self, 'matmul_precision') and hasattr(torch, 'set_float32_matmul_precision'):
        ...
        if self.use_gpu and getattr(self, 'use_torch_compile', False) and hasattr(torch, 'compile'):
    ...
    def extract(self, im: torch.Tensor):
        ...
        inference_context = getattr(torch, 'inference_mode', torch.no_grad) if getattr(self, 'use_inference_mode', True) else torch.no_grad
        autocast_context = nullcontext
        if self.use_gpu and getattr(self, 'use_amp', False) and hasattr(torch, 'autocast'):
...
class PersonEcoLiteFeature(MultiFeatureBase):
    """Lightweight backbone specialized for person tracking."""

    def __init__(self, output_layers, net_path=None, use_gpu=True, *args, **kwargs):
        super(PersonEcoLiteFeature, self).__init__(*args, **kwargs)
        ...
        self.output_layers = list(output_layers)
        self.use_gpu = use_gpu
        self.net_path = 'person_ecolite/person_ecolite_person_mix_v1.pth' if net_path is None else net_path

    def initialize(self):
        if isinstance(self.pool_stride, int) and self.pool_stride == 1:
            self.pool_stride = [1] * len(self.output_layers)

        self.layer_stride = {'stem': 2, 'stage1': 4, 'stage2': 8, 'stage3': 8, 'stage4': 16, 'fc': None}
        self.layer_dim = {'stem': 24, 'stage1': 32, 'stage2': 48, 'stage3': 96, 'stage4': 96, 'fc': None}
        ...
        for net_path in net_path_full:
            try:
                self.net = person_ecolite(self.output_layers, path=net_path)
                break
            except Exception:
                pass
        if self.net is None:
