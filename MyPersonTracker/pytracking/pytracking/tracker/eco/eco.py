...
class ECO(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        x_mat = self.build_centered_sample_matrix(x)
        self.projection_matrix = self.load_pretrained_projection_matrix()
        if self.projection_matrix is None or self.should_fallback_projection(x_mat, self.projection_matrix):
            cov_x = x_mat @ x_mat.t()
    ...
    def load_pretrained_projection_matrix(self):
        projection_path = self.params.get('pretrained_projection_path', None)
        ...
        if not projection_path.is_file():
            ...
            for root in root_paths:
                candidate = Path(root) / projection_path
                if candidate.is_file():
                    projection_path = candidate
                    break
        if not projection_path.is_file():
            raise FileNotFoundError('Pretrained projection matrix not found: {}'.format(projection_path))

        payload = torch.load(str(projection_path), map_location=self.params.device)
        matrices = payload.get('projection_matrix', payload)
        if not isinstance(matrices, (list, tuple)) or len(matrices) != len(self.compressed_dim):
            raise ValueError('Invalid projection matrix payload in {}'.format(projection_path))

        loaded = []
        for matrix, cdim in zip(matrices, self.compressed_dim):
            tensor = torch.as_tensor(matrix, device=self.params.device, dtype=torch.float32)
            if tensor.ndim != 2 or tensor.shape[1] < int(cdim):
                raise ValueError('Projection matrix shape mismatch in {}'.format(projection_path))
            loaded.append(tensor[:, :int(cdim)].contiguous().clone())

        return TensorList(loaded)

    def build_centered_sample_matrix(self, x: TensorList) -> TensorList:
        x_mat = TensorList([e.permute(1, 0, 2, 3).reshape(e.shape[1], -1).clone() for e in x])
        x_mat -= x_mat.mean(dim=1, keepdim=True)
        return x_mat

    def should_fallback_projection(self, x_mat: TensorList, projection_matrix: TensorList) -> bool:
        if not self.params.get('projection_fallback', False):
            return False

        threshold = self.params.get('projection_fallback_ratio_threshold', 0.60)
        if isinstance(threshold, (list, tuple)):
            thresholds = list(threshold)
        else:
            thresholds = [float(threshold)] * len(x_mat)

        for feat_mat, proj, feat_threshold in zip(x_mat, projection_matrix, thresholds):
            total_var = float((feat_mat * feat_mat).sum().item())
            if total_var <= 0:
                continue
            captured_var = float(((proj.t() @ feat_mat) ** 2).sum().item())
            capture_ratio = captured_var / total_var
            if capture_ratio < float(feat_threshold):
                return True
