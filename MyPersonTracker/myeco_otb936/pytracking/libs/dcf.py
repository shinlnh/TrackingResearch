...
def get_reg_filter(sz: torch.Tensor, target_sz: torch.Tensor, params):
    ...
    reg_window_dft = torch.view_as_real(torch.fft.rfftn(reg_window, dim=(-2, -1))) / sz.prod()
    ...
    reg_window_sparse = torch.fft.irfftn(
        torch.view_as_complex(reg_window_dft.contiguous()),
        s=tuple(int(v) for v in sz.long().tolist()),
        dim=(-2, -1),
    )
    if torch.is_complex(reg_window_sparse):
        reg_window_sparse = reg_window_sparse.real
