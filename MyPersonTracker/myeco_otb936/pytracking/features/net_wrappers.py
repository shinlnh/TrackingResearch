...
class NetWrapper:
    ...
    def __init__(self, net_path, use_gpu=True, initialize=False, cache_network=True, **kwargs):
    ...
    def load_network(self):
        ...
        if self.cache_network:
            try:
                cache_key = (self.net_path, self.use_gpu, tuple(sorted(self.net_kwargs.items())))
            except TypeError:
                cache_key = None

        if cache_key is not None and cache_key in NetWrapper._network_cache:
            self.net = NetWrapper._network_cache[cache_key]
        else:
            self.net = load_network(self.net_path, **self.net_kwargs)
            if self.use_gpu:
                self.cuda()
            if cache_key is not None:
                NetWrapper._network_cache[cache_key] = self.net
...
class NetWithBackbone(NetWrapper):
    ...
    def __init__(self, net_path, use_gpu=True, initialize=False, image_format='rgb',
        ...
        super().__init__(net_path, use_gpu, initialize, cache_network=cache_network, **kwargs)
