...
def load_weights(net, path, strict=True):
    checkpoint_dict = _torch_load(path)
...
def torch_load_legacy(path):
    ...
    # Load network
    checkpoint_dict = _torch_load(path, map_location='cpu')
...
def _torch_load(path, map_location=None):
    ...
    # that include constructor objects. Force legacy behavior when available.
    kwargs = {}
