...
$outputDir = Join-Path $repoRoot "OtherTracker\lasot\lasot936\CF2"
$resultsDir = Join-Path $outputDir "tracking_results\CF2"
...
$cleanupTargets = @(
    ...
    (Join-Path $outputDir "cf2_matlab_summary.csv")
)
...
results_dir = pathlib.Path("OtherTracker/lasot/lasot936/CF2/tracking_results/CF2")
...
$env:CF2_USE_GPU = '1'
$matlabCmd = "addpath(fullfile(pwd,'OtherTracker','CF2')); run_cf2_lasot_headtail40(fullfile(pwd,'ls','lasot'), fullfile(pwd,'OtherTracker','lasot','lasot936','headtail40_sequences.txt'), fullfile(pwd,'OtherTracker','lasot','lasot936','CF2'));"

& matlab -batch $matlabCmd 2>&1 | Tee-Object -FilePath $logPath

$runExitCode = $LASTEXITCODE

if ($runExitCode -ne 0) {
    $errorLine = "ERROR: MATLAB error Exit Status: 0x{0:X8}" -f $runExitCode
    $errorLine | Tee-Object -FilePath $logPath -Append
    exit $runExitCode
}
...
tracker = SimpleNamespace(
    ...
    run_id=None,
    display_name="CF2",
    results_dir=str(repo / "OtherTracker" / "lasot" / "lasot936" / "CF2" / "tracking_results" / "CF2"),
)

report_name = "lasot_headtail40_cf2_othertracker"
...
summary_path = repo / "OtherTracker" / "lasot" / "lasot936" / "CF2" / "summary.csv"
