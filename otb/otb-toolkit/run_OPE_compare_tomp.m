...
% Baseline trackers shown in common OTB OPE plots.
baseline_names = { ...
    'MDNet', 'CCOT', 'DeepSRDCF', 'SRDCFdecon', 'SRDCF', 'Staple', ...
    'HDT', 'CF2', 'LCT', 'CNN-SVM', 'SAMF', 'MEEM', 'DSST', 'KCF'};

% Add your exported trackers here.
custom_names = {'MyToMPTracker', 'ToMP_plus3'};
...
% Use the workspace OTB dataset by default.
...
dst_fig = fullfile(fig_dir, 'success_plot_tomp_compare.png');
dst_fig_run812 = fullfile(fig_dir, 'success_plot_mytomptracker_run812.png');
if exist(src_fig, 'file') == 2
    copyfile(src_fig, dst_fig);
    copyfile(src_fig, dst_fig_run812);
    fprintf('Saved comparison figure: %s\n', dst_fig);
    fprintf('Saved run812 figure: %s\n', dst_fig_run812);
end
