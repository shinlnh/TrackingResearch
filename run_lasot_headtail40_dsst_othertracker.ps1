...
$outputDir = Join-Path $repoRoot "OtherTracker\lasot\lasot936\DSST"
$resultsDir = Join-Path $outputDir "tracking_results\DSST"
...
$cleanupTargets = @(
    ...
    (Join-Path $outputDir "dsst_matlab_summary.csv")
)
...
results_dir = pathlib.Path("OtherTracker/lasot/lasot936/DSST/tracking_results/DSST")
...
cmd /c "set PYTHONUNBUFFERED=1 && matlab -batch ""addpath(fullfile(pwd,'OtherTracker','DSST')); run_dsst_lasot_headtail40;"" 2>&1" |
...
tracker = SimpleNamespace(
    ...
    run_id=None,
    display_name="DSST",
    results_dir=str(repo / "OtherTracker" / "lasot" / "lasot936" / "DSST" / "tracking_results" / "DSST"),
)

report_name = "lasot_headtail40_dsst_othertracker"
...
summary_path = repo / "OtherTracker" / "lasot" / "lasot936" / "DSST" / "summary.csv"
