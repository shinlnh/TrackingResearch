...
$outputDir = Join-Path $repoRoot "OtherTracker\lasot\lasot936\Staple"
$resultsDir = Join-Path $outputDir "tracking_results\Staple"
...
$cleanupTargets = @(
    ...
    (Join-Path $outputDir "staple_matlab_summary.csv")
)
...
results_dir = pathlib.Path("OtherTracker/lasot/lasot936/Staple/tracking_results/Staple")
...
cmd /c "set PYTHONUNBUFFERED=1 && matlab -batch ""addpath(fullfile(pwd,'OtherTracker','Staple')); run_staple_lasot_headtail40;"" 2>&1" |
...
tracker = SimpleNamespace(
    ...
    run_id=None,
    display_name="Staple",
    results_dir=str(repo / "OtherTracker" / "lasot" / "lasot936" / "Staple" / "tracking_results" / "Staple"),
)

report_name = "lasot_headtail40_staple_othertracker"
...
summary_path = repo / "OtherTracker" / "lasot" / "lasot936" / "Staple" / "summary.csv"
