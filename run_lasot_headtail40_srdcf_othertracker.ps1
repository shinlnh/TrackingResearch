...
$outputDir = Join-Path $repoRoot "OtherTracker\lasot\lasot936\SRDCF"
$resultsDir = Join-Path $outputDir "tracking_results\SRDCF"
...
$cleanupTargets = @(
    ...
    (Join-Path $outputDir "srdcf_matlab_summary.csv")
)
...
results_dir = pathlib.Path("OtherTracker/lasot/lasot936/SRDCF/tracking_results/SRDCF")
...
cmd /c "set PYTHONUNBUFFERED=1 && matlab -batch ""addpath(fullfile(pwd,'OtherTracker','SRDCF')); run_srdcf_lasot_headtail40;"" 2>&1" |
...
tracker = SimpleNamespace(
    ...
    run_id=None,
    display_name="SRDCF",
    results_dir=str(repo / "OtherTracker" / "lasot" / "lasot936" / "SRDCF" / "tracking_results" / "SRDCF"),
)

report_name = "lasot_headtail40_srdcf_othertracker"
...
summary_path = repo / "OtherTracker" / "lasot" / "lasot936" / "SRDCF" / "summary.csv"
